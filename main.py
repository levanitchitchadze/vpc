...
def create_subnet(vpc_id, cidr_block, subnet_name):
    ...
    check_if_exist= ec2_client.describe_subnets(
        Filters=[
            {"Name": "vpc-id", "Values": [vpc_id]},
            {"Name": "cidrBlock", "Values": [cidr_block]}
        ]
    ).get("SubnetId")
...
def vpc_and_more(private_cidr_block,private_subnet_name,public_cidr_block,public_subnet_name):

    vpc_id= list_vpcs()[0].get("VpcId")

    # create private subnet
    subnet_id =create_subnet(vpc_id, private_cidr_block, private_subnet_name)
    print(subnet_id)
    rtb_id = create_route_table_without_route(vpc_id)
    associate_route_table_to_subnet(rtb_id, subnet_id)
    #
    # # create public subnet
    subnet_id = create_subnet(vpc_id, public_cidr_block, public_subnet_name)
...
def main():
    ...
    private_subnet_name='private_sub_10'

    public_cidr_block = "10.0.13.0/24"
    public_subnet_name='public_sub_14'

    vpc_and_more(private_cidr_block, private_subnet_name, public_cidr_block, public_subnet_name)

    #
    private_cidr_block = "10.0.7.0/24"
    private_subnet_name='private_sub_101'
